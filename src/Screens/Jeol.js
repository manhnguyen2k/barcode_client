...
import { JeolGenator, JeolRead } from "../api/Jeol.api";
...
const Jeol = () => {
    ...
    const exportToExcel = async () => {
        try {
            ...
            setLoading(true)
            const res = await JeolGenator(dataBarcode)
            setLoading(false)
    ...
    const handleSubmit = async () => {
      ...
      //  console.log(typeof(so_thu_tu_lo_lon))
        ...
        try {
            setLoading(true)
            setIsExport(false)
            const data = await JeolGenator(dataBarcode)
    ...
    const handleRead = async () => {
        ...
        try {
            setLoading(true)
            const data = await JeolRead(barcode)
            setLoading(false)
            // console.log(data)
            if (data.data) {
                    ...
                    <div className="container_barcode">
                        ...
                        <div className="barcode_item">
                            ...
                            <select value={bottleSizeCode} onChange={(e) => setBottleSizeCode(e.target.value)}>
                                <option value={2}>20ml (square)</option>
